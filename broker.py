...
class PubSubBroker:
    ...
    def run(self):
        ...
        def init():
            ...
            if self.__log_queue:
                self.logger = get_queue_logger(
                    self.__log_queue, "PubSub", "PubSub", self.__log_level, self.__log_filter)
        ...
        try:
            while True:
                readable, _, _ = select.select(
                    [self.__broker_channel._reader, self.__publish_channel._reader], [], [])
                if self.__broker_channel._reader in readable:
                    ...
                    if isinstance(sub_unsub, SubscriptionRequest):
                        ...
                        raise TypeError(
                            "SubscriptionRequest or SubscriptionCancelation expected")
